...
GITHUB_TOKEN   = os.environ.get("GITHUB_TOKEN", "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")
...
def github_upload():
    if not GITHUB_TOKEN:
        return jsonify({"error": "GITHUB_TOKEN is not configured on the server"}), 503
    ...
    # Keep original filename; sanitise unsafe chars only
    ...
    # If file already exists, fetch its SHA so we can overwrite it
    sha = None
    check = http_req.get(api_url, headers=headers, timeout=10)
    ...
    resp = http_req.put(api_url, headers=headers, json=payload, timeout=30)
    ...
    return jsonify({"error": resp.json().get("message", "GitHub API error")}), resp.status_code
